...
all_attn  |>
  ...
  theme(strip.background = element_rect(fill="#cccccc", 
        ...
        strip.text=element_text(color="black", 
                                face = "bold",
                                size = 11),
        plot.margin = margin(10, 10, 10, 70), # t, r, b, l 
        panel.spacing = unit(0.4, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 0.8),
        panel.background = element_rect(fill = "white", color = NA)) 
...
I show the results across all layers.
...
ggsave(filename = file.path(res_path, "dist_bio_attn_pos_all.png"), width = 6, height = 4, dpi = 300)
```
...
We want to look at the attention for each feature type across the different layers. 
