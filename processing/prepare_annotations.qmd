...
This code aim to process an XML file to extract annotations from the human reference proteome in UniProt and structure them into a well-organized table. XML files often contain rich metadata, but extracting relevant information requires parsing and transformation into a more readable format such as a CSV or TSV table. The resulting annotation table will provide a structured view of the extracted data, making it easier to analyze and use in the downstream analysis. Here, I chose to simply save the R object.
...
```{r}
saveRDS(feature_data, file = "../data/complete/annotations/interim/anno_initial.rds")
```
...
I will filter out TrEMBL sequences for both the feature and length data generated above.
...
```{r}
saveRDS(feature_data, file = "../data/complete/annotations/interim/anno_filter1.rds")
```

## Filter annotation types

I am not interested in all annotation types from UniProt. Thus, I read in a file describing whether to keep a annotation or not.
...
```{r}
saveRDS(feature_data, file = "../data/complete/annotations/interim/anno_filter2.rds")
```

## Filter annotations beyond position 1024

The ESM-2 model will only consider proteins of 1024 amino acids or less. Let's get the percentage of sequences longer than 1024.
...
For the long sequences, I will crop them and only consider the first 1024 positions. However, I wish to avoid cropping a sequence in the middle of an annotation. If found to be in the middle of an annotation for position 1024, the edge will be define as the position prior to the start of the annotated region or site.
...
I join the adjusted length with the filtered annotations.
...
Finally, I remove the annotations that does not appear in the cropped sequence. Furthermore, I remove annotations without start and end positions.
...
```{r}
saveRDS(feature_data, file = "../data/complete/annotations/processed/anno.rds")
```
